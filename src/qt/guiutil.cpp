...
{
    // return if URI is not valid or is no SDC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert sdc:// to sdc:
    //
    //    Cannot handle this later, because sdc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open sdc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SDC.lnk";
}
...
{
    return GetAutostartDir() / "sdc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a sdc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sdc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add sdc app to startup item list
