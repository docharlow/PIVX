...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSdcAmount"))
        settings.setValue("nAnonymizeSdcAmount", 1000);

    nAnonymizeSdcAmount = settings.value("nAnonymizeSdcAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSdcAmount"))
        SoftSetArg("-anonymizesdcamount", settings.value("nAnonymizeSdcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sdc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSdcAmount:
            return QVariant(nAnonymizeSdcAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSdcAmount:
            nAnonymizeSdcAmount = value.toInt();
            settings.setValue("nAnonymizeSdcAmount", nAnonymizeSdcAmount);
            emit anonymizeSdcAmountChanged(nAnonymizeSdcAmount);
            break;
        case CoinControlFeatures:
